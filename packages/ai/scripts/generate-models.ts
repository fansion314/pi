...
const GITHUB_COPILOT_THINKING_LEVEL_OVERRIDES = {
	...
	// Preserve the 2026-09-22 fallback's effort levels once models.dev lists this model.
	"claude-opus-5.5": {
		off: null,
		minimal: null,
		low: "low",
...
function applyOpenAICompletionsTranscriptMetadata(model: Model<Api>): void {
	...
	const isTextOnly =
		...
		(model.provider === "deepseek-completions" && model.id === "deepseek-v4-pro") ||
...
async function generateModels() {
	...
	const deepseekCompletionsModels: Model<"openai-completions">[] = [
		{
			id: "deepseek-flash",
	...
	const deepseekResponsesModels: Model<"openai-responses">[] = deepseekCompletionsModels.map((model) => ({
		...model,
		api: "openai-responses",
		provider: "deepseek",
		thinkingLevelMap: DEEPSEEK_RESPONSES_THINKING_LEVEL_MAP,
		compat: {
			supportsDeveloperRole: false,
	...
	allModels.push(...deepseekResponsesModels, ...deepseekCompletionsModels);
