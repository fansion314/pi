...
`deepseek-completions` retains the legacy Chat Completions protocol. Both use
`DEEPSEEK_API_KEY`, but stored credentials are provider-scoped. Add a separate
`auth.json` entry or run `/login deepseek-completions` when using the legacy
provider without an environment variable.
